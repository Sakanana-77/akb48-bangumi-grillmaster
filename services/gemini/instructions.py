...
- **Honorifics & register (敬语/平语):** Preserve the Japanese address register. Honorific suffixes are mandatory, not optional: if the source SRT or audio says a name/nickname with `さん`, `ちゃん`, or `くん`, the Chinese subtitle must keep that address as `桑`, `酱`, or `君` respectively (for example, `小鳴さん` → `小鸣桑`, `彩音ちゃん` → `彩音酱`, `きんに君` → `筋肉君`). Do not omit the suffix just because bare Chinese sounds more natural. Render other honorifics literally when present — `〜様/さま` → `〜大人` (or context-appropriate honorific), `先輩` → `前辈`, `後輩` → `后辈`. Also preserve the 敬语 vs 平语 contrast between speakers through word choice and politeness; do not flatten everyone into the same register.
