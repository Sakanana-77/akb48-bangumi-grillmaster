...
The user message includes a 固定词汇表 — hand-curated source→target mappings
filtered to entries with at least one name appearing in this episode's
inputs. It has two sections: 〔艺人/组合〕 where each act is a `・组合：` line
(its 组合 name; `・（单人）` instead for a solo act) followed by indented `·`
member lines, and 〔节目/单元/品牌/术语〕, a flat `-` list. Within any line
" / " separates alias forms of ONE entity mapping to a single Simplified
Chinese target. These are the highest-priority truth for naming/term
decisions:

- If a glossary target contains Traditional Chinese characters, convert it to
  Mainland China Simplified Chinese before writing it into `characters`,
  `proper_nouns`, `glossary`, `catchphrases`, or subtitles. The glossary fixes
  entity choice, romanization, and span, not Traditional script.
- A `・组合：` line is BOTH a normal mapping (use it when the 组合 name is
  ...
  when the audio / SRT / on-screen text / surrounding 组合 context confirms
...
- Classify each entry into the appropriate output field:
  - A person under 〔艺人/组合〕 → `characters` (`name_jp` = the canonical/
  ...
  - A 组合 name that is not a single person, or any 〔节目/单元/品牌/术语〕
...
- These mappings OVERRIDE the proper-noun localization hierarchy. Do NOT
  ...
  "official" China title seems to exist.
...
The user message includes a 固定词汇表 — the COMPLETE hand-curated
source→target mapping (NOT filtered to this episode), in two sections:
〔艺人/组合〕 where each act is a `・组合：` line (or `・（单人）` for a solo
act) followed by indented `·` member lines, and 〔节目/单元/品牌/术语〕, a
flat `-` list. Within any line " / " separates alias forms of ONE entity
mapping to one Simplified Chinese target.
...
- A `・组合：` line is both a normal mapping (when the 组合 name is spoken)
  ...
  target only when audio / SRT / on-screen text / 组合 context confirms it is
  that act.
...
### TARGET LANGUAGE (HIGHEST PRIORITY)

All JSON values ending with _zh MUST use Mainland China Simplified Chinese.
...
- **summary**: ~200 Simplified Chinese chars describing the show's overall premise, segment structure, and comedic style based on the audio vibe. Helps downstream workers set tone.
...
- **Proper-noun localization policy**: Aim for information parity — a Chinese viewer should recover as much of the naming intent (meaning, wordplay, kanji core, member/place names, loanword parts) as a Japanese viewer gets from the original; a bare phonetic transliteration that hides that intent is a last resort, not the default. For program/segment/talent/group names and other proper nouns, take the FIRST tier that fits:
  ...
  Hard rules: never fabricate a stylized Simplified Chinese retitle; do not literal-translate a nickname kana name (bad `"松井ケムリ": "松井烟"`); never romanize a token already written in kanji in the source unless the glossary maps that exact kanji form to romaji — tier 2 overrides tier 4; if unsure about an official China rendering, skip tier 1.
...
- **tone_notes**: ~100 Simplified Chinese chars on register/energy derived directly from listening to the audio. Call out which speakers use 敬语 vs 平语 with each other (so the downstream translator preserves politeness asymmetry), and any signature address habits (e.g., "主持人总以 XX 桑 称呼嘉宾"). E.g., "节奏明快，以关西腔话家常为主，吐槽直接，赞美便当与酒时情感真挚。高潮在花瓣飘入的一刻胜败感强烈，翻译时语尾保留关西腔爽快感。"

- **segment_summaries**: EXACTLY one entry per chunk boundary provided. `from_index` and `to_index` must equal the boundary values. `summary` (~350 Simplified Chinese chars) describes what happens in that local range so the chunk worker has narrative context without reading other chunks.
...
- Use Mainland China Simplified Chinese conventions in all `*_zh` fields.
...
－
—
Treat them as immediate interruptions.
Keep the leading symbol.
Do not add brackets.
...
- **Target:** Simplified Chinese (Mainland China). Natural spoken Simplified Chinese Mandarin suitable for variety shows.
- **Proper nouns:** Follow the pre-pass `characters` and `proper_nouns` mappings exactly. For a new proper noun not in the briefing, aim for naming-information parity (not a bare transliteration) and take the FIRST tier that fits:
  1. **Established Simplified Chinese/common Chinese rendering** — only when verifiable from on-screen captions, program text, or clearly stable Chinese usage.
  2. **Recoverable kanji for people** — a kana stage name that maps to a known real-name/surname kanji uses Simplified Chinese kanji.
  3. **Parseable source → keep the naming structure** — full Chinese when it still reads like a name (`"プロレス"` in a stage name → `"摔角"`), or semantic/kanji core + kept loanword (`海鸥Mental`, `Double东`), or recovered surname/place/allusion kanji (`加贺屋`, `兰奢待`). Do NOT over-localize into a plain noun/sentence/invented nickname.
  ...
  Hard rules: never fabricate a stylized Simplified Chinese retitle; do not literal-translate a nickname kana name; never romanize a token already written in kanji in the source unless the glossary maps that exact kanji form to romaji (tier 2 overrides tier 4).
...
- **Honorifics & register (敬语/平语):** Preserve the Japanese address register. Honorific suffixes are mandatory, not optional: if the source SRT or audio says a name/nickname with `さん`, `ちゃん`, or `くん`, the Chinese subtitle must keep that address as `桑`, `酱`, or `君` respectively (for example, `なるさん` → `naru桑`, `ずっきーちゃん` → `Zukky酱`, `きんに君` → `筋肉君`). Do not omit the suffix just because bare Chinese sounds more natural. Render other honorifics literally when present — `〜様/さま` → `〜大人` (or context-appropriate honorific), `先輩` → `前辈`, `後輩` → `后辈`. Also preserve the 敬语 vs 平语 contrast between speakers through word choice and politeness; do not flatten everyone into the same register.
...
- **Vocal onomatopoeia:** When a block is just a speaker's raw vocalization (laughter, gasps, screams — e.g. `ハハハ`, `ああ`, `ええっ`), either transliterate into a natural Chinese counterpart that fits the moment (`哈哈哈`, `啊啊啊`, `诶`) or leave the text line empty. Do NOT replace it with a descriptive label such as `（笑声）` / `（惊呼）` — that style belongs to scene-sound blocks, not to a speaker's actual utterance.
