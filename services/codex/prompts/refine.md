...
- Latin-name spacing (Netflix): a Latin-letter or mixed Chinese-Latin proper noun (use `.pre_pass/pre_pass.json` proper_nouns/characters to identify the unit) is one indivisible token. Ensure one half-width space between that unit and adjacent Han/kana; none inside it (`水川Katamari`), none against punctuation or line edges, none on pure Han/kanji names (`鎌鼬的山內`). Repair missing/wrong spacing and collapse double spaces, but do not strip spacing that is already correct and never split a unit across a line break.
